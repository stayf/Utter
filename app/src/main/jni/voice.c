#include "com_stayfprod_utter_service_VoiceController.h"
#include <jni.h>
#include <ogg/ogg.h>
#include <stdio.h>
#include <opus.h>
...
JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_startRecord(JNIEnv *env, jclass class, jstring path) {
...
JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame(frameBytes, len);
}

JNIEXPORT void Java_com_stayfprod_utter_service_VoiceController_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}
...
JNIEXPORT jlong Java_com_stayfprod_utter_service_VoiceController_getTotalPcmDuration(JNIEnv *env, jclass class) {
    return _totalPcmDuration;
}

JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_getFinished(JNIEnv *env, jclass class) {
    return finished;
}

JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_getSize(JNIEnv *env, jclass class) {
    return size;
}

JNIEXPORT jlong Java_com_stayfprod_utter_service_VoiceController_getPcmOffset(JNIEnv *env, jclass class) {
    return pcmOffset;
}

JNIEXPORT void Java_com_stayfprod_utter_service_VoiceController_readOpusFile(JNIEnv *env, jclass class, jobject buffer, jint capacity) {
    jbyte *bufferBytes = (*env)->GetDirectBufferAddress(env, buffer);
    fillBuffer(bufferBytes, capacity);
}

JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_seekOpusFile(JNIEnv *env, jclass class, jfloat position) {
    return seekPlayer(position);
}

JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_openOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT void Java_com_stayfprod_utter_service_VoiceController_closeOpusFile(JNIEnv *env, jclass class) {
    cleanupPlayer();
}

JNIEXPORT int Java_com_stayfprod_utter_service_VoiceController_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
