...
public class Switch extends View implements Checkable {
    ...
    protected void applyStyle() {
        setCheckedImmediately(false);
        mMaxAnimDuration = 200;
        mTrackSize = Constant.DP_14;
        mThumbRadius = Constant.DP_10;

        mShadowSize = Constant.DP_2;
        ...
        if (mTrackSize < 0)
            mTrackSize = Constant.DP_2;

        if (mThumbRadius < 0)
            mThumbRadius = Constant.DP_8;
    ...
    private void startAnimation() {
        if (getHandler() != null) {
            resetAnimation();
            mRunning = true;
            getHandler().postAtTime(mUpdater, SystemClock.uptimeMillis() + 16L);
        } else
            mThumbPosition = mChecked ? 1f : 0f;
        invalidate();
    }
